...
class BatteryDualDisassembly(Node):
    def __init__(self, node_name="master_node_dis10"):
    ...
    def find_target_candidate_with_retry(self, target, retries=3, use_base_nearest=False):
        ...
        for _ in range(retries):
            for candidate in candidates:
                request_target = f"base_nearest:{candidate}" if use_base_nearest else candidate
                p = self.call(self.cli_v1, GetTargetPose.Request(target_color=request_target))
                if p.success:
                    if candidate != target:
    ...
    def run_disassembly_once(self, name, top_target, top_label, bottom_label, yaw_offset=0.0):
        self.get_logger().info(f"{name} 협조 분해 시작: 2층 {top_label} / 1층 {bottom_label}")
        self.move_both_home_pose()
        self.set_gripper(self.cli_g1, False)
        self.set_gripper(self.cli_g2, False)

        if not self.robot1_top_pick(top_target, top_label, yaw_offset=yaw_offset): return False
    ...
    def run_battery_once(self):
        return self.run_disassembly_once("배터리", "2x2_yellow", "2x2 노랑", "2x2 파랑")

    def run_magnet_once(self):
        return self.run_disassembly_once("자석", "2x2_blue", "2x2 파랑", "2x2 빨강")

    def run_estop_once(self):
        return self.run_disassembly_once("E-stop", "2x2_red", "2x2 빨강", "2x4 노랑", yaw_offset=0.0)
    ...
    def run_ice_cream_once(self):
        return self.run_four_layer_once("아이스크림", [
            ...
            (3, "2x2_blue", "2x2 파랑", 90.0, "DROP2"),
            (3, "2x2_red", "2x2 빨강", -90.0, "DROP3"),
    ...
    def run_big_tree_once(self):
        ...
        if not self.robot1_pick_layer_drop_top_then_robot2_drop(
            ...
            bottom_label="2층 초록", robot2_drop_slot="DROP2", yaw_offset=90.0
        ):
            return False
